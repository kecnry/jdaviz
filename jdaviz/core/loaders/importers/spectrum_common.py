...
class SpectrumInputExtensionsMixin(VuetifyTemplate, HubListener):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(self.input, fits.HDUList):
            ...
            ext_options = [{'label': 'specutils.Spectrum',
                            'name': 'specutils.Spectrum',
                            'name_ver': None,
                            'index': 0,
                            'obj': self.input}]
        else:
    ...
    def asdf_roman_is_valid_flux(item):
    ...
    def is_valid_unc(self, item):
        ...
        if self.input_type == 'specutils:spectrum':
            return getattr(self.input, 'uncertainty', None) is not None
        raise NotImplementedError("Unknown input type")
    ...
    def asdf_roman_is_valid_unc(self, item):
        # TODO: should this instead have options just to include or not include and pull from the flux extension?
    ...
    def is_valid_mask(self, item):
        ...
        if self.input_type == 'specutils:spectrum':
            return getattr(self.input, 'mask', None) is not None
        raise NotImplementedError("Unknown input type")
    ...
    @cached_property
    def spectrum(self):
        # each input_type needs to assign flux, flux_unit
        if self.input_type == 'fits:hdulist':
            hdulist = self.input
        ...
        try:
            spectrum = Spectrum(flux=flux * flux_unit, uncertainty=unc,
            ...
            try:
                spectrum = Spectrum.read(self._parser.input)
            except Exception:
                # specutils.Spectrum reader would fail, so use no WCS
                spectrum = Spectrum(
                        flux=flux * flux_unit, uncertainty=unc,
            ...
            flux_error = extension.get("flux_error", None) # TODO: don't allow a separate uncertainty extension?
            ...
            # TODO: move this to the unc_extension section
            ...
            for ext in ('SCI', 'FLUX', 'PRIMARY', 'DATA'):  # In priority order
                ...
                for cunit_num in (3, 1):
                    ...
                    if cunit_key in hdr and 'WAV' in hdr[ctype_key]:
                        ...
                        found_target = True
                        break
        if target_wave_unit == spectrum.spectral_axis.unit:
            target_wave_unit = None
        if (target_wave_unit is None) and (target_flux_unit is None):  # Nothing to convert
            ...
            new_scp = spectrum.with_spectral_axis_unit(target_wave_unit)
        elif target_wave_unit is None:  # Convert flux only and only PIX2 stuff
            new_sp = spectrum.with_flux_unit(target_flux_unit, equivalencies=_eqv_flux_to_sb_pixel())
        else:  # Convert both
            new_sp = spectrum.with_spectral_axis_and_flux_units(
                target_wave_unit, target_flux_unit, flux_equivalencies=_eqv_flux_to_sb_pixel())
        if target_wave_unit is not None:
            new_sp.meta['_orig_spec'] = spectrum
        ...
        if _get_celestial_wcs(spectrum.wcs) is not None:
            new_sp.meta['_orig_spatial_wcs'] = _get_celestial_wcs(spectrum.wcs)
