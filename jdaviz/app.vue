              ...
              <v-expansion-panels accordion multiple focusable flat tile v-model="state.tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in state.tray_items" :key="index">
                  <div v-if="trayItemVisible(trayItem, state.tray_items_filter)">
                    <v-expansion-panel-header @click="() => {if (!state.tray_items_open.includes(index)) {reactivate_plugin(trayItem.name)} else {update_tray_item_active(trayItem.name)}}">
                      <j-tooltip v-if="state.tray_items_active.includes(trayItem.name)" span_style="position: absolute; top: 14px; left: 5px" tooltipcontent="this plugin is being kept active by another instance (popout, inline) and/or the keep_active option, click to deactivate all instances">
