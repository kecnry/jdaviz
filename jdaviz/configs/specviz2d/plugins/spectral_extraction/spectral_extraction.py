...
class SpectralExtraction(PluginTemplateMixin):
    ...
    * :meth:`export_extract_spectrum`
    ...
    @observe('plugin_opened', 'persistent_previews', 'interactive_extract')
    def _update_plugin_marks(self, *args):
        if not self._do_marks:
            return

        if not self.show_live_previews:
    ...
    def marks(self):
        ...
        show_live_previews = self.show_live_previews
        self._marks = {'trace': PluginLine(viewer2d, visible=show_live_previews),
                       'ext_lower': PluginLine(viewer2d, visible=show_live_previews),
                       'ext_upper': PluginLine(viewer2d, visible=show_live_previews),
                       'bg1_center': PluginLine(viewer2d, visible=show_live_previews,
                                                line_style='dotted'),
                       'bg1_lower': PluginLine(viewer2d, visible=show_live_previews),
                       'bg1_upper': PluginLine(viewer2d, visible=show_live_previews),
                       'bg2_center': PluginLine(viewer2d, visible=show_live_previews,
                                                line_style='dotted'),
                       'bg2_lower': PluginLine(viewer2d, visible=show_live_previews),
                       'bg2_upper': PluginLine(viewer2d, visible=show_live_previews)}
        # NOTE: += won't trigger the figure to notice new marks
        viewer2d.figure.marks = viewer2d.figure.marks + list(self._marks.values())

        self._marks['extract'] = PluginLine(viewer1d, visible=show_live_previews)
        self._marks['bg_spec'] = PluginLine(viewer1d, visible=show_live_previews, stroke_width=1)  # noqa
    ...
    def _interaction_in_trace_step(self, event={}):
        if not self.show_live_previews or not self._do_marks:
            return
